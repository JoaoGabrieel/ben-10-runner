...
func _ready() -> void:	
	...
	if ground and ground.has_node("Sprite2D"):
		ground_height = ground.get_node("Sprite2D").texture.get_height()
	
	if game_over_hud and game_over_hud.has_node("Button"):
		if not game_over_hud.get_node("Button").pressed.is_connected(new_game):
			game_over_hud.get_node("Button").pressed.connect(new_game)
		
	if has_node("Pause/Resume"):
		if not $Pause/Resume.pressed.is_connected(_on_resume_pressed):
...
func _process(_delta):
	...
	if is_slow_mo:
		...
		ambiente.modulate = ambiente.modulate.lerp(Color(1, 1, 1), 10 * _delta)
	...
	if game_running: 
		...
		# Progresso do Omnitrix
		if omnitrix_state == OmnitrixState.LOCKED:
